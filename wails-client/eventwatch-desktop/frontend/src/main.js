...
<main class="grid">
  ...
  <section class="card" id="cheatsheet-card">
    ...
    <label>Filter <input id="cheat-filter" placeholder="pr, incr, deploy_started, chat/general, ..." autocapitalize="off" autocorrect="off" spellcheck="false" /></label>
    <div id="cheat-groups"></div>
  </section>
...
// -- cheatsheet card --
// In-UI reference for every event type. Each row's "Inject →" writes into
// the Publish card's fields (pub-topic / pub-type / pub-payload) so the
// user can review + fire without typing.
const CHEAT_EXAMPLES = [
  // pr
  { group: 'pr', label: 'pr_opened — new PR', topic: 'pr/octo/hello/1', type: 'pr_opened', payload: {title:'Add feature X', author:'alice', base:'main', head:'abc123'} },
  { group: 'pr', label: 'pr_review_requested', topic: 'pr/octo/hello/1', type: 'pr_review_requested', payload: {reviewer:'bob'} },
  { group: 'pr', label: 'pr_reviewed — approved', topic: 'pr/octo/hello/1', type: 'pr_reviewed', payload: {state:'approved'} },
  { group: 'pr', label: 'pr_reviewed — changes requested', topic: 'pr/octo/hello/1', type: 'pr_reviewed', payload: {state:'changes_requested'} },
  { group: 'pr', label: 'check_run_completed — success', topic: 'pr/octo/hello/1', type: 'check_run_completed', payload: {conclusion:'success', name:'unit-tests'} },
  { group: 'pr', label: 'check_run_completed — failure', topic: 'pr/octo/hello/1', type: 'check_run_completed', payload: {conclusion:'failure', name:'unit-tests'} },
  ...
  { group: 'pr', label: 'pr_closed', topic: 'pr/octo/hello/1', type: 'pr_closed', payload: {} },
  // build
  ...
  { group: 'build', label: 'step_finished — success', topic: 'build/ci/42', type: 'step_finished', payload: {step:'compile', status:'success'} },
  { group: 'build', label: 'build_finished — success', topic: 'build/ci/42', type: 'build_finished', payload: {status:'success'} },
  // deploy
  { group: 'deploy', label: 'deploy_started v42', topic: 'deploy/prod/api', type: 'deploy_started', payload: {version:'v42', env:'prod', service:'api'} },
  ...
  { group: 'deploy', label: 'deploy_finished', topic: 'deploy/prod/api', type: 'deploy_finished', payload: {status:'success'} },
  // job
  { group: 'job', label: 'job_started', topic: 'job/reindex-1', type: 'job_started', payload: {name:'reindex'} },
  { group: 'job', label: 'job_progress 50%', topic: 'job/reindex-1', type: 'job_progress', payload: {percent:50, eta_seconds:30} },
  ...
  { group: 'chat', label: 'msg_posted', topic: 'chat/general', type: 'msg_posted', payload: {id:'m1', user:'alice', text:'hey team'} },
  ...
  { group: 'str', label: 'str_set "" (still "set")', topic: 'str/name', type: 'str_set', payload: {value:''}, note: 'value="" and exists=true — different from delete' },
  { group: 'str', label: 'str_delete', topic: 'str/name', type: 'str_delete', payload: {}, note: 'value="" and exists=false' },
  // int (math)
  { group: 'int', label: 'int_set 100', topic: 'int/counter', type: 'int_set', payload: {value:100} },
  { group: 'int', label: 'int_incr +1 (default)', topic: 'int/counter', type: 'int_incr', payload: {}, note: 'no delta key → defaults to +1' },
  { group: 'int', label: 'int_incr +5', topic: 'int/counter', type: 'int_incr', payload: {delta:5} },
  { group: 'int', label: 'int_incr -3 (subtracts)', topic: 'int/counter', type: 'int_incr', payload: {delta:-3}, note: 'negative delta subtracts' },
  { group: 'int', label: 'int_decr -1 (default)', topic: 'int/counter', type: 'int_decr', payload: {}, note: 'no delta key → defaults to -1' },
  { group: 'int', label: 'int_decr +5', topic: 'int/counter', type: 'int_decr', payload: {delta:5} },
  { group: 'int', label: 'int_incr from unset (starts at 0)', topic: 'int/hits', type: 'int_incr', payload: {delta:7}, note: 'no need to int_set 0 first' },
  { group: 'int', label: 'int_set 0 (explicit zero)', topic: 'int/counter', type: 'int_set', payload: {value:0}, note: 'exists=true (differs from int_delete)' },
  { group: 'int', label: 'int_delete (value=0, exists=false)', topic: 'int/counter', type: 'int_delete', payload: {} },
  // time (math)
  { group: 'time', label: 'time_now (server clock)', topic: 'time/last_seen', type: 'time_now', payload: {}, note: 'uses server clock — no client skew' },
  { group: 'time', label: 'time_set RFC3339', topic: 'time/last_seen', type: 'time_set', payload: {value:'2026-09-01T12:00:00Z'} },
  { group: 'time', label: 'time_add +1h', topic: 'time/last_seen', type: 'time_add', payload: {seconds:3600} },
  { group: 'time', label: 'time_add -60s', topic: 'time/last_seen', type: 'time_add', payload: {seconds:-60}, note: 'negative seconds subtract' },
  { group: 'time', label: 'time_delete', topic: 'time/last_seen', type: 'time_delete', payload: {} },
];
const CHEAT_GROUPS = [
  { key:'pr',     label:'PR',     hint:'GitHub-style PR lifecycle' },
  { key:'build',  label:'Build',  hint:'CI build + step timings' },
  { key:'deploy', label:'Deploy', hint:'Deployment with versions + health' },
  { key:'job',    label:'Job',    hint:'Long-running job with progress + logs' },
  ...
  { key:'time',   label:'Time',   hint:'Scalar timestamp with add/sub' },
];
const cheatOpen = { pr: true, int: true };   // default-open groups

function injectExample(ex) {
  $('pub-topic').value = ex.topic;
  $('pub-type').value = ex.type;
  $('pub-payload').value = JSON.stringify(ex.payload);
  document.getElementById('pub-topic').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function renderCheatsheet() {
  const filter = ($('cheat-filter').value || '').toLowerCase().trim();
  const matches = (ex) => !filter ||
    ex.label.toLowerCase().includes(filter) ||
    ex.type.toLowerCase().includes(filter)  ||
    ex.topic.toLowerCase().includes(filter);
  const groupsEl = $('cheat-groups');
  groupsEl.innerHTML = '';
  for (const g of CHEAT_GROUPS) {
    const items = CHEAT_EXAMPLES.filter((e) => e.group === g.key && matches(e));
    if (items.length === 0) continue;
    const isOpen = filter ? true : (cheatOpen[g.key] ?? false);
    const wrapper = document.createElement('div');
    wrapper.className = 'cheat-group';
    const header = document.createElement('button');
    header.className = 'cheat-group-header';
    header.innerHTML = `${isOpen ? '▼' : '▶'} <b>${g.label}</b> <span class="hint">— ${g.hint} · ${items.length}</span>`;
    header.onclick = () => { cheatOpen[g.key] = !isOpen; renderCheatsheet(); };
    wrapper.appendChild(header);
    if (isOpen) {
      const tbl = document.createElement('table');
      tbl.className = 'cheat-table';
      const tbody = document.createElement('tbody');
      for (const ex of items) {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td class="cheat-label">${escapeHtml(ex.label)}</td>
          <td class="cheat-payload"><code>${escapeHtml(ex.type)}</code><br />
            <code class="hint">${escapeHtml(ex.topic)}</code>
            ${ex.note ? `<div class="hint">↳ ${escapeHtml(ex.note)}</div>` : ''}
          </td>
          <td class="cheat-json"><code>${escapeHtml(JSON.stringify(ex.payload))}</code></td>
          <td><button>Inject →</button></td>`;
        tr.querySelector('button').onclick = () => injectExample(ex);
...
$('cheat-filter').addEventListener('input', renderCheatsheet);
renderCheatsheet();
